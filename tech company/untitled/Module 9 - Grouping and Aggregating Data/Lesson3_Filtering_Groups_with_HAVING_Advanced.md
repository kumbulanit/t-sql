...
SELECT d.DepartmentName,
    ...
    FORMAT(
        (SUM(ISNULL(d.Budget, 0)) - SUM(ISNULL(p.ActualCost, 0))) / NULLIF(COUNT(DISTINCT e.EmployeeID), 0), 
        'C0'
    ...
    CASE 
        ...
        WHEN (SUM(ISNULL(d.Budget, 0)) - SUM(ISNULL(p.ActualCost, 0))) * 100.0 / NULLIF(SUM(d.Budget), 0) >= 30
...
HAVING 
    ...
    AND (
        ...
        SUM(d.Budget) / COUNT(DISTINCT e.EmployeeID) >= 1500000
        
        -- OR profit margin exceeds 25%
        OR ((SUM(ISNULL(d.Budget, 0)) - SUM(ISNULL(p.ActualCost, 0))) * 100.0) / NULLIF(SUM(d.Budget), 0) >= 25
        ...
        OR SUM(d.Budget) >= 10000000
    )

ORDER BY SUM(d.Budget) / COUNT(DISTINCT e.EmployeeID) DESC;
...
SELECT 
    ...
    -- FINANCIAL VALUE
    FORMAT(SUM(d.Budget), 'C0') AS TotalClientValue,
    FORMAT(AVG(d.Budget), 'C0') AS AvgProjectValue,
    ...
    CASE 
        WHEN SUM(d.Budget) >= 10000000 
             AND COUNT(p.ProjectID) >= 15
             AND COUNT(CASE WHEN p.IsActive = 'Completed' AND p.ActualEndDate <= p.PlannedEndDate THEN 1 END) * 100.0 / 
                 NULLIF(COUNT(CASE WHEN p.IsActive = 'Completed' THEN 1 END), 0) >= 85
        THEN '👑 STRATEGIC ENTERPRISE CLIENT - Crown Jewel'
        WHEN SUM(d.Budget) >= 5000000 
             AND (SUM(ISNULL(d.Budget, 0)) - SUM(ISNULL(p.ActualCost, 0))) * 100.0 / NULLIF(SUM(d.Budget), 0) >= 25
        THEN '💎 PREMIUM CLIENT - High Value & Profitable'
        WHEN COUNT(p.ProjectID) >= 10 
             AND AVG(d.Budget) >= 400000
        THEN '🏆 MAJOR CLIENT - Consistent High-Value Work'
        WHEN SUM(d.Budget) >= 2000000
    ...
    CASE 
        WHEN SUM(d.Budget) >= 10000000 THEN 'Dedicated C-Suite Relationship + Account Team'
        WHEN SUM(d.Budget) >= 5000000 THEN 'Senior Partner + Dedicated Account Manager'
        WHEN SUM(d.Budget) >= 2000000 THEN 'Partner-Level Account Management'
        WHEN AVG(d.Budget) >= 500000 THEN 'Senior Account Manager'
...
ORDER BY SUM(d.Budget) DESC, AVG(d.Budget) DESC;
```
